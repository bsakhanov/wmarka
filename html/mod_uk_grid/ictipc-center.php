...
<div class="mod_uk_grid<?php echo $grid_class, $classes; ?>   uk-margin-top uk-flex uk-flex-center uk-grid uk-border-rounded  " data-uk-grid<?php echo $grid_params, $hm_param; ?> uk-scrollspy="cls: uk-animation-fade; target: > div > .uk-card; delay: 500; repeat: true" uk-height-match="target: > div > .uk-card">
    <?php
    ...
    <div class=" uk-card uk-card-default uk-card-body uk-portfolio uk-text-primary uk-text-center uk-border-rounded" >
        
		    <?php if ($item->icon) { ?>
            <span class="uk-text-primary uk-padding-remove uk-margin-auto" uk-icon="icon: <?php echo $item->icon; ?>; ratio: 5"></span>
            <?php } ?>
            <?php if ($item->img) { ?>
            ...
            <?php if ($item->link) { ?>
            <div class="uk-margin <?php echo trim(' ' . $item->al_content); ?>"><a class="uk-button uk-button-primary uk-button-small uk-border-rounded  uk-text-bold " href="#<?php echo $item->link_text; ?>" uk-toggle><?php echo $item->link_text; ?>  </a></div>
            <?php } ?>
